...
// 4. ACCUMULATE 实现
template <typename Iterator, typename T, typename BinaryOp = std::plus<>>
T accumulate(Iterator first, Iterator last, T init, BinaryOp op = BinaryOp()) {
    while (first != last) {
        init = op(init, *first);
        ++first;
    }
...
std::pair<Iterator1, Iterator2> mismatch(Iterator1 first1, Iterator1 last1, Iterator2 first2) {
...
void nth_element(Iter first, Iter nth, Iter last, Compare comp = Compare()) {
    while (first != last) {
        Iter pivot = first + (last - first) / 2;
        std::iter_swap(pivot, last - 1); // 将pivot交换到末尾
        auto p = std::move(*(last - 1));

        Iter mid = my::partition(first, last - 1, [&p, &comp](const auto& x) {
            return comp(x, p);
            });
...
void sort(Iter first, Iter last, Compare comp = Compare()) {
    if (first >= last) return;  // 递归终止条件：区间为空或单元素

    // 选择中间元素作为基准（pivot）
    Iter pivot = first + (last - first) / 2;
    std::iter_swap(pivot, last - 1);  // 将基准交换到末尾
    auto p = std::move(*(last - 1));  // 基准值

    // 分区操作：[first, mid) < p，[mid, last) >= p
    Iter mid = my::partition(first, last - 1, [&p, &comp](const auto& x) {
        return comp(x, p);
        });
    ...
    // 递归排序左右子区间
    my::sort(first, mid, comp);
    my::sort(mid + 1, last, comp);
}

// 10. NEXT_PERMUTATION 实现
